...
export default function StudentPage() {
  ...
  const isFormValid =
    fullName.trim() && grade.trim() && school.trim() && isEmailValid;
  useEffect(() => {
    if (step !== "exam") return;
  ...
  const inputClass = (invalid: boolean) =>
    `mt-2 w-full rounded-xl border bg-[#f8fafc] px-4 py-3 text-sm font-medium text-[#1f2a44] outline-none transition ${invalid ? "border-[#ef4444] focus:border-[#ef4444]" : "border-[#d5dbe7] focus:border-[#4ca3f0]"}`;
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-[#6a7390]">
                  Шалгалт эхлэхийн өмнө мэдээллээ бөглөнө үү.
                </p>
              </div>
              <span className="rounded-full bg-[#eef2ff] px-3 py-1 text-xs font-semibold text-[#3b4a86]">
                Алхам 1/2
              </span>
            </div>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <div>
                <label className="text-sm font-semibold text-[#3a4763]">
                  Овог нэр
                </label>
                <input
                  className={inputClass(touched.fullName && !fullName.trim())}
                  placeholder="Элзий-Орших Түвшин"
                  type="text"
                  value={fullName}
              ...
              <div>
                ...
                <input
                  className={inputClass(touched.grade && !grade.trim())}
                  placeholder="8A"
                  type="text"
                  value={grade}
                ...
                />
                {touched.grade && !grade.trim() && (
                  <p className="mt-2 text-xs font-medium text-[#ef4444]">
                    Ангиа заавал бөглөнө үү.
                  </p>
                )}
              </div>
              <div>
                ...
                <input
                  className={inputClass(touched.school && !school.trim())}
                  placeholder="Сүхбаатар сургууль"
                  type="text"
                  value={school}
                  onChange={(e) => setSchool(e.target.value)}
                  onBlur={() => markTouched("school")}
                />
                {touched.school && !school.trim() && (
                  <p className="mt-2 text-xs font-medium text-[#ef4444]">
                    Сургуулиа заавал бөглөнө үү.
                  </p>
                )}
              </div>
            </div>
            <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
              <p className="text-xs text-[#6a7390]">
                Мэдээллээ зөв бөглөсний дараа шалгалтаа эхлүүлнэ.
              </p>
              <button
