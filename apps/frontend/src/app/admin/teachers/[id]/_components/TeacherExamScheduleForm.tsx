...
import { calculateDuration, scheduleStatuses, statusLabels } from "../_utils/exam-schedule";
...
export function TeacherExamScheduleForm({
  ...
  return (
    <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            Багшийн шалгалтын хуваарийг анги, сэдэв, цагийн мэдээлэлтэйгээр удирдана.
          </p>
        </div>
        {values.id ? (
          <button
      ...
      <form onSubmit={onSubmit} className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <div className="rounded-xl border border-dashed border-zinc-300 bg-zinc-50 px-4 py-3 text-sm text-zinc-600">
          Үргэлжлэх хугацаа цагийн сонголтоос автоматаар тооцогдоно. Урьдчилсан дүн:
          <span className="ml-1 font-semibold text-zinc-900">{durationPreview}</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 xl:col-span-2 xl:justify-end">
          ...
          >
            {isPending ? "Хадгалж байна..." : values.id ? "Өөрчлөлт хадгалах" : "Хуваарь нэмэх"}
          </button>
        </div>
      </form>
    </section>
