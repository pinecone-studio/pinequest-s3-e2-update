...
const inputClass =
  "h-14 w-full rounded-[14px] border border-gray-200 tracking-wider bg-white px-4 text-base text-gray-900 shadow-sm " +
  ...
  async function resendMfaCode() {
    ...
    try {
      if (selectedMfaStrategy === "phone_code") {
        const { error } = await si.mfa.sendPhoneCode();
        setMfaHint(error ? clerkErrMessage(error) : "SMS код дахин илгээлээ.");
      } else if (
  ...
  if (isSignedIn) {
    return (
      <div className="flex min-h-screen flex-col items-stretch justify-center bg-[#f4f4ee] px-0 py-0 sm:items-center sm:p-4">
        <div
          className={`${authPanelClass} items-center justify-center px-5 sm:px-8`}
        >
    ...
    <div className="flex min-h-screen flex-col items-stretch justify-center bg-[#f4f4ee] px-0 py-0 sm:items-center sm:p-4">
      <div className={authPanelClass}>
        <header className="flex shrink-0 items-center px-5 pb-4 pt-[calc(env(safe-area-inset-top)+1rem)] sm:px-8">
          ...
          <h1 className="min-w-0 flex-1 text-center text-lg font-bold  tracking-widest text-gray-900">
        ...
        <div className="flex min-h-0 flex-1 flex-col px-5 pb-6 sm:px-8 sm:pb-8">
          <div className="flex min-h-0 flex-1 flex-col justify-center overflow-y-auto py-2">
              ...
              !(signIn as SignInWithMfa).mfa ? (
                <div className="space-y-4 text-center">
                  <p className="text-sm text-red-600" role="alert">
                    Нэмэлт баталгаажуулалтыг энэ хувилбар дэмжихгүй байна.
                    Хуудсыг дахин ачаалж, эсвэл тохиргоог шалгана уу.
                  </p>
                  <button
                    type="button"
              ...
              >
                ...
                <div>
                  <label
                    ...
                    className="sr-only tracking-wider"
                  >
                    Нууц үг
                  </label>
                ...
                <AuthButton
                  type="submit"
                  disabled={loading}
                  className={`mt-1 h-14 tracking-widest rounded-[14px] text-base cursor-pointer font-bold ${primaryBlue}`}
                >
            ...
            {secondFactorPending ? null : (
              <>
                <Link
                  ...
                  className="mt-5 block text-center text-sm font-medium tracking-wider text-gray-600 underline-offset-2 hover:text-gray-900 hover:underline"
                >
                ...
                <AuthButton
                  ...
                  disabled={loading}
                  onClick={onGoogle}
                  className="cursor-pointer mt-6 h-14 rounded-[14px] tracking-widest border-[#29B6FF] bg-white text-base font-semibold text-[#29B6FF] shadow-none hover:bg-[#f0f9ff]"
                >
                  Google-ээр нэвтрэх
                </AuthButton>
                ...
                <Link
                  href={signUpHref}
                  className="mt-8 block text-center text-base font-bold tracking-widest text-gray-800 underline-offset-2 hover:underline"
                >
                  Бүртгүүлэх
                </Link>
              </>
            ...
            <div className="mt-8 flex items-center justify-center gap-2 pb-[env(safe-area-inset-bottom)]">
              ...
              />
              <span className="text-lg font-black tracking-wider text-[#11161d]">
                UPDATE
              </span>
            </div>
          </div>
