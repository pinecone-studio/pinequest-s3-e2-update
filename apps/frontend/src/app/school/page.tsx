    ...
    <div className="space-y-6 text-2">
      <section className="rounded-2xl border border-[#dbe5f0] bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <h2 className="text-4 font-bold text-[#0f172a]">
              Сургуулийн самбар
            </h2>
            <p className="mt-1 text-2 text-zinc-600">
              Шалгалтын төлөв, эрсдэл, гүйцэтгэлийг нэг дэлгэцнээс хянах
              school-level харагдац.
            </p>
          </div>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-6">
          {summaryCards.map((card) => (
            <article
              key={card.label}
              className="rounded-xl border border-[#e6edf5] bg-[#f8fbff] p-4"
            >
            ...
            {pendingActions.map((item) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span
                    className={`rounded-full px-2 py-0.5 text-2 font-semibold ${
                      item.severity === "high"
        ...
        <article className="rounded-2xl border border-[#dbe5f0] bg-white p-5 shadow-sm">
          ...
          <ul className="mt-4 space-y-3">
            {aiSuggestions.map((item) => (
              ...
              >
                <p className="font-medium text-zinc-900">{item.title}</p>
                <p className="mt-1 text-2 text-zinc-600">{item.problem}</p>
                <p className="mt-1 text-2 text-zinc-500">
