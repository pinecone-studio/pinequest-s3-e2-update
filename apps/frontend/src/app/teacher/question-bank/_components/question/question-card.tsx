...
/** Demo-only heart totals (not from API). Each id maps to a stable value in ~20–46. */
function placeholderHeartCount(questionId: string): number {
	let h = 2166136261;
	for (let i = 0; i < questionId.length; i++) {
		h ^= questionId.charCodeAt(i);
		h = Math.imul(h, 16777619);
	}
	return 20 + (Math.abs(h) % 27);
}
			...
			<div className="mt-[12px] flex items-center justify-between text-[#7B7B7B]">
				...
				>
					...
					<span className={cn(isLiked ? "text-[#e11d48]" : "text-[#525252]")}>
						{placeholderHeartCount(question.id)}
					</span>
				</button>
