  ...
  return (
    <div className="space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-slate-700">Ухаалаг шүүлтүүр</p>
        <button
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <select
          ...
          onChange={(event) => onChange("subject", event.target.value as QuestionFilters["subject"])}
        ...
        <select
          className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm"
          onChange={(event) =>
            onChange("difficulty", event.target.value as QuestionFilters["difficulty"])
          }
        ...
        <select
          className="h-10 rounded-lg border border-slate-200 bg-white px-3 text-sm"
          onChange={(event) => onChange("status", event.target.value as QuestionFilters["status"])}
