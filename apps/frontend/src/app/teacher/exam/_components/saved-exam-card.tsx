...
import { BookCopy, CheckCircle2, Eye, SendHorizontal, Trash2 } from "lucide-react";
import {
  Select,
  SelectContent,
  SelectItem,
...
export function SavedExamCard({
  ...
  const availableClasses = teacherClasses.filter((klass) => klass.grade === savedExam.grade);
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          <div className="min-w-52">
            <Select onValueChange={onSelectClass} value={selectedClassId ?? undefined}>
              ...
              <SelectContent>
                {availableClasses.length > 0 ? (
                  availableClasses.map((klass) => <ClassOption key={klass.id} klass={klass} />)
                ) : (
          ...
          <ActionButton icon={<SendHorizontal className="mr-2 h-4 w-4" />} kind="primary" label="Илгээх" onClick={onSend} />
          <ActionButton icon={<Eye className="mr-2 h-4 w-4" />} label="Нээх" onClick={onOpen} />
          <ActionButton icon={<BookCopy className="mr-2 h-4 w-4" />} label="Засварлах" onClick={onOpen} />
          <ActionButton icon={<Trash2 className="mr-2 h-4 w-4" />} kind="danger" label="Устгах" onClick={onDelete} />
        </div>
      </div>
    </article>
  );
    ...
    <div className="mt-3 space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {sentClassIds.map((classId) => {
          ...
          return klass ? <span className="rounded-full bg-[#eef6ff] px-3 py-1 text-xs font-semibold text-[#2f66b9]" key={classId}>Илгээсэн: {klass.name}</span> : null;
        })}
      </div>
    </div>
  );
...
function ClassOption({ klass }: { klass: TeacherClass }) {
  return <SelectItem value={klass.id}>{klass.name} · {klass.studentCount} сурагч</SelectItem>;
}
...
function ActionButton({
  ...
  return <button className={`inline-flex h-11 items-center rounded-2xl border px-4 text-sm font-semibold transition ${styles}`} onClick={onClick} type="button">{icon}{label}</button>;
}
